...
**`traefik/dynamic/custom-hosts.yml`** — vanity hostnames **outside** the apps zones (e.g. `www.<DOMAIN>` → `k3d-ingress`, apex `<DOMAIN>` → permanent redirect to `www`). Management API hostname overrides only update the Kubernetes Ingress `Host`; without a matching outer Traefik router here, public requests still 404. After editing, recreate Traefik so the entrypoint re-processes templates: `docker compose up -d --force-recreate traefik`.
