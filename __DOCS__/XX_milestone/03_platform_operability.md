...
┌─────────────────────────────────────────────────────────────┐
│                     Operator surface                        │
│   dsoctl CLI  ──────────────────►  Management API (GraphQL) │
│   Operator console (milestone 02)   /admin mutations        │
└───────────────────────────┬─────────────────────────────────┘
                            │
                    ┌───────▼────────┐
                    │   OpenBao      │
                    ...
                    │ └─────────────┘│
                    └───────┬────────┘
                            │  reads at startup / on signal
          ┌─────────────────┼──────────────────────────────┐
          │                 │                              │
    ┌─────▼─────┐    ┌──────▼──────┐              ┌───────▼──────┐
    │ Keycloak  │    │  Management │              │  oauth2-proxy│
    │           │    │    API      │              │  Traefik     │
    │ reads DB  │    │ reads GitLab│              │  cloudflared │
    │ password  │    │ PAT, API key│              │              │
...
```
┌─────────────┐       dsoctl secret rotate       ┌──────────────┐
│   ACTIVE    │──────────────────────────────────►│  GENERATING  │
└─────────────┘                                   └──────┬───────┘
       ▲                                                  │ new value generated
       │ old value decommissioned                         ▼
       │                                          ┌──────────────┐
┌──────┴──────┐                                   │   PUSHING    │
│  RETIRING   │◄──────────────────────────────────│  (to target) │
└─────────────┘  push verified, reload complete   └──────┬───────┘
                                                         │ push confirmed
                                                         ▼
                                                  ┌──────────────┐
                                                  │  RELOADING   │
