...
The Traefik dashboard (`https://traefik.devops.yourdomain.com`), the operator console (`https://console.devops.yourdomain.com`), and other labeled operator surfaces don't have built-in OIDC support. They're protected by `oauth2-proxy`, which sits in front of them and enforces Keycloak login before allowing access. By default only users in the Keycloak **`admins`** group (JWT `groups` claim) may use those panels; configure **`OAUTH2_PROXY_ALLOWED_GROUPS`** in `.env` to change or widen that list (see [Environment variables](../01_infra/02_env.md#oauth2-proxy-allowed-groups)).

**Dev and staging application URLs** (`https://<slug>.dev.apps.<DOMAIN>`, `https://<slug>.stg.apps.<DOMAIN>`) are **ungated by default** — only specific paths that opt in (typically a frontend's UI, e.g. `/app`) are gated by a second instance, **`oauth2-proxy-apps`**, at the outer Traefik layer. Any devops-realm user in **`admins`** or **`users`** (default for new accounts) can browse a gated path after platform login. APIs and everything else on the same dev/stg host — and all of production (`*.apps.<DOMAIN>`) — stay public at this layer; apps are expected to enforce their own auth for those paths. See [oauth2-proxy tiers](08_oauth2_proxy_tiers_and_forwardauth.md#what-exists-today) and [how to gate a new app path](08_oauth2_proxy_tiers_and_forwardauth.md#how-to-gate-a-new-app-path).
