...
if ! vault_read_json "${INDEX_PATH}" | jq -e '._index_json' >/dev/null 2>&1; then
...
vault_read_json "${INDEX_PATH}" | jq -c '._index_json | fromjson | .profiles[]?' | while IFS= read -r profile; do
  ...
  if [ "${delivery}" = "raw_file" ]; then
    content="$(vault_read_json "${vault_path}" | jq -r '._raw_content // empty')"
    if [ -z "${content}" ]; then
  ...
  if [ "${delivery}" = "dotenv_build_args" ]; then
    ...
    if [ -n "${filename}" ] && [ "${filename}" != "null" ]; then
      dest="$(workspace_dest_path "${workspace}" "${filename}")"
      mkdir -p "$(dirname "${dest}")"
      vault_read_json "${vault_path}" | jq -r 'to_entries[] | select(.key != "_raw_content") | "\(.key)=\(.value)"' > "${dest}"
      stage_build_file "${workspace}" "${filename}" "${dest}"
      if [ -z "${workspace}" ]; then
