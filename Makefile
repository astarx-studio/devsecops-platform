...
help:
	...
	@echo "  make smoke-deploy  - ./bootstrap/smoke-deploy.sh (smoke-api + smoke-web + smoke-mono + smoke-sonar; ARGS='--cleanup' tears down with no-trace verify)"
	@echo "  make smoke-cleanup - ./bootstrap/smoke-cleanup.sh (hard-delete all 4 smoke projects + verify no trace)"
...
reset-sonarqube:
	@cd "$(ROOT)" && bash ./bootstrap/reset-sonarqube.sh
