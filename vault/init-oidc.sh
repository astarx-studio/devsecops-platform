#!/bin/sh
# ==========================================================================
# Vault OIDC Auth Method Initialization
# ==========================================================================
# Enables OIDC authentication in Vault, pointing to the Keycloak devops realm.
# Idempotent: "vault auth enable" fails gracefully if already enabled.
#
...
#   VAULT_EXTERNAL_URL       - Vault external URL for redirect URIs (https://vault.devops.yourdomain.com)
# ==========================================================================
...
until wget --spider --quiet "${VAULT_ADDR}/v1/sys/health" 2>/dev/null; do
  echo "[DEBUG] Vault not ready, retrying in 2s..."
  sleep 2
done
echo "[INFO] Vault is ready."

echo "[INFO] Enabling OIDC auth method..."
vault auth enable oidc 2>/dev/null || echo "[WARN] OIDC auth method already enabled (skipping)."
...
echo "[INFO] Vault OIDC configuration complete."
